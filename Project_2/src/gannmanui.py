...
class GannManUi:
    ...
    def options_menu(self):
        ...
        while (not optionSelected):
            ...
            character = getwch()
            if character == "w":
                self.menuIndexPointer = (self.menuIndexPointer - 1) % menuLength
            elif character == "s":
                self.menuIndexPointer = (self.menuIndexPointer + 1) % menuLength
            elif character == '\r':
                optionSelected = True
    ...
    def input_run_scenario_menu(self):
        ...
        if confirmation == 'y':
            ...
            self.gannMan.create_gann(name, networkDims, hiddenActivationFunc, outputActivationFunc,
                                     lossFunc, optimizer, optimizerParams, learningRate, weightInitType, weightInit,
                                     dataSource, dataSourceParas,
            ...
            self.gannMan.run_gann(epochs=int(selectEpochs), showInterval=None,
                                  validationInterval=int(selectValInt), bestK=selectBestK,
                                  mapBatchSize=selectMapBatchSize,
                                  mapLayers=selectMapLayers, mapDendrograms=selectMapDendrograms,
                                  displayWeights=selectDisplayWeights,
